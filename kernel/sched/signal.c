...
int kill_task(struct task_struct *task, int sig)
{
    ...
    if (!task || !task->signals) return -1;
    
    printk(KERN_DEBUG "Sending signal %d to PID %d\n", sig, task->pid);
...
int sigaction_syscall(int sig, const struct sigaction *act, struct sigaction *oldact)
{
    struct task_struct *task = get_current();
...
pid_t do_waitpid(pid_t pid, int *wstatus, int options)
{
    ...
    printk(KERN_DEBUG "waitpid: PID %d waiting for %d\n", current->pid, pid);
